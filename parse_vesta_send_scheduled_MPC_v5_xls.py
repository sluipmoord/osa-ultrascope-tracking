...
def dms2dec(dms_str):
    ...
    (degree, minute, second, frac_seconds, junk) = re.split('\D+', dms_str,
                                                            maxsplit=4)
    ...
    return sign * (int(degree) + float(minute) / 60 +
                   float(second) / 3600 + float(frac_seconds) / 3600)
...
def hms2dec(dms_str):
    ...
    (hours, minute, second, frac_seconds, junk) = re.split('\D+', dms_str,
                                                           maxsplit=4)
...
def main():
    def push_mount_messages_queue(message, alt, az):
        ...
        scheduler.enterabs(time.mktime(t.timetuple()), 1,
                           push_mount_messages_queue,
                           ('Running again', alt, az))
    ...
    # !!!! change this range to fit the numbe rof columns in the final M
    rows = range(3, 300)
    ...
    for row in rows:
        ...
        (year, month, day) = [sheet.cell(row, 0).value,
                              sheet.cell(row, 1).value,
                              sheet.cell(row, 2).value]

        (hour, minute, second) = [sheet.cell(row, 3).value,
